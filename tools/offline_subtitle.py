...
流程:视频 → SenseVoice + VAD 识别 → LLM 翻译 → 生成 SRT+ASS →(可选)硬压进视频。
...
def main() -> None:
    ...
    p.add_argument("--model", default="sensevoice", help="兼容参数，v1.4.1 固定使用 SenseVoice")
    ...
    print(f"[1/4] 识别中(SenseVoice,{device})…")
    def prog(done, total):
